document.addEventListener('DOMContentLoaded', function () {
  ...
  var modalFormCard     = $('modalFormCard');
  var modalFormBloqueado = $('modalFormBloqueado');
  var btnEnviar         = modalFormCard ? modalFormCard.querySelector('.btn-enviar') : null;
  var btnLoginRequerido = $('btnLoginRequerido');

  function abrirModalNoticia() {
    var sesion = JSON.parse(sessionStorage.getItem('sesionActiva') || 'null');
    if (modalFormCard && modalFormBloqueado) {
      if (sesion) {
        modalFormCard.classList.remove('oculto');
        modalFormBloqueado.classList.add('oculto');
      } else {
        modalFormCard.classList.add('oculto');
        modalFormBloqueado.classList.remove('oculto');
      }
    }
  ...
  if (btnLoginRequerido) {
    btnLoginRequerido.addEventListener('click', function () {
      if (modalOverlay) modalOverlay.classList.remove('activo');
  ...
  function construirNoticiaGeneroItemHTML(n, i) {
    return (
      '<div class="noticia-red-item' + (i === 0 ? ' noticia-clickeable' : '') + '"' + (i === 0 ? ' id="noticiaRedAndrea"' : '') + '>' +
        '<img class="noticia-red-img" src="img/generolista' + (i + 1) + '.png" alt="Noticia"/>' +
        '<div class="noticia-red-texto">' +
          '<h3 class="noticia-red-titulo">' + n.titulo + '</h3>' +
          '<p class="noticia-red-desc">' + n.desc + '</p>' +
          '<a href="#" class="noticia-red-leermas">Leer más (' + n.tiempo + ' min de lectura) <i class="fas fa-chevron-right" style="font-size:10px;"></i></a>' +
  ...
  function abrirNoticiaAndrea() {
    noticiaCompletaOverlay.innerHTML =
      ...
      '<div class="noticia-completa-grid">' +
        '<img class="noticia-completa-img" src="img/genero1-principal.png" alt="Andrea D\'Atri y Clara Muzzio"/>' +
        '<div class="noticia-completa-texto">' +
